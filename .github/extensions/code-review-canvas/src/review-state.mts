...
let reviewedDatabase: DatabaseSync | null = null;
...
function getReviewedDatabase() {
    ...
    `);
    const columns = reviewedDatabase.prepare("PRAGMA table_info(reviewed_files)").all() as Array<{ name: string }>;
...
export function getReviewedFiles(reviewKeysByFile: Record<string, string>) {
...
export function setReviewedFiles(reviewKeysByFile: Record<string, string>, files: string[], reviewed: boolean) {
    const entries = files
