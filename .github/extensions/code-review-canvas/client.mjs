import { failedTargetLabel, fileNameFromPath, loadedTargetLabelForBranch, makeWhitespaceVisible } from "./client-text.mjs";
...
const splitter = document.querySelector("#left-pane-splitter");
const mainSplitter = document.querySelector("#left-main-splitter");
const rangeToggle = document.querySelector("#range-toggle");
const rangeList = document.querySelector("#range-list");
const checklist = document.querySelector(".checklist");
const expandRange = document.querySelector("#expand-range");
const targetTabs = document.querySelectorAll(".target-tab");
const targetPanels = document.querySelectorAll(".target-panel");
const prNumberText = document.querySelector("#pr-number-text");
const baselineText = document.querySelector("#baseline-text");
const loadedTargetStatuses = document.querySelectorAll(".loaded-target-status");
const overviewButton = document.querySelector("#overview-button");
const overviewSpinner = document.querySelector("#overview-spinner");
const loadPrButton = document.querySelector("#load-pr-button");
const checkoutPrButton = document.querySelector("#checkout-pr-button");
const loadBranchButton = document.querySelector("#load-branch-button");
const rangeText = document.querySelector("#range-text");
const rangeClearButton = document.querySelector("#range-clear-button");
const fileTree = document.querySelector("#file-tree");
const filesSpinner = document.querySelector("#files-spinner");
const diffTitle = document.querySelector("#diff-title");
const diffView = document.querySelector("#diff-view");
const commentsSplitter = document.querySelector("#comments-splitter");
...
const commentContext = document.querySelector("#comment-context");
...
const diffLayoutButtons = document.querySelectorAll(".diff-layout-button");
const diffSideButtons = document.querySelectorAll(".diff-side-button");
const diffScrollLockButton = document.querySelector("#diff-scroll-lock");
const diffHelpButton = document.querySelector("#diff-help-button");
const diffHelpDialog = document.querySelector("#diff-help-dialog");
const diffReviewedToggle = document.querySelector("#diff-reviewed-toggle");
const diffIgnoreWhitespaceButton = document.querySelector("#diff-ignore-whitespace");
const diffVisibleWhitespaceButton = document.querySelector("#diff-visible-whitespace");
...
setLoadedTargetStatus(targetMode === "pr"
    ? "Loading PR #" + loadedBranch.trim().replace(/^#/, "") + "..."
    : loadedTargetSuccessLabel, targetMode === "pr" ? "loading" : "");
...
function applyFontScaleToOverviewFrame() {
    const frame = diffView.querySelector(".overview-frame");
    if (frame) {
...
function diffSideUnsafeCSS() {
    ...
    if (diffStyle === "split" && !diffScrollLocked && diffSide === "both") {
        rules.push(':host { display: block; height: 100%; min-height: 0; }', '[data-diff-type="split"][data-overflow="scroll"] { height: 100%; min-height: 0; overflow: hidden !important; }', '[data-diff-type="split"][data-overflow="scroll"] > code[data-code] { align-content: start; box-sizing: border-box; height: 100%; min-height: 0; overflow: scroll !important; overscroll-behavior: contain; padding-right: 28px; scrollbar-color: color-mix(in srgb, var(--diffs-fg), transparent 72%) color-mix(in srgb, var(--diffs-bg), transparent 70%); scrollbar-gutter: stable; scrollbar-width: auto; }', '[data-diff-type="split"][data-overflow="scroll"] > code[data-code]::-webkit-scrollbar { width: 12px !important; height: 12px !important; }', '[data-diff-type="split"][data-overflow="scroll"] > code[data-code]::-webkit-scrollbar-track { background-color: color-mix(in srgb, var(--diffs-bg), transparent 70%) !important; }', '[data-diff-type="split"][data-overflow="scroll"] > code[data-code]::-webkit-scrollbar-thumb { background-color: color-mix(in srgb, var(--diffs-fg), transparent 72%) !important; background-clip: content-box; border: 2px solid transparent; border-radius: 999px; }', '[data-diff-type="split"][data-overflow="scroll"] > code[data-code]::-webkit-scrollbar-thumb:horizontal { min-width: 2rem; }', '[data-diff-type="split"][data-overflow="scroll"] > code[data-code]::-webkit-scrollbar-thumb:vertical { min-height: 2rem; }', '[data-diff-type="split"][data-overflow="scroll"] > code[data-code]::-webkit-scrollbar-corner { background-color: color-mix(in srgb, var(--diffs-bg), transparent 70%) !important; }', '[data-diff-type="split"][data-overflow="scroll"] > code[data-code] :is([data-content-buffer], [data-gutter-buffer="buffer"]) { display: none !important; }');
    }
    if (diffSide === "left") {
        rules.push('[data-diff-type="split"] { grid-template-columns: 1fr !important; }', 'code[data-additions] { display: none !important; }', 'code[data-deletions] :is([data-content-buffer], [data-gutter-buffer="buffer"]) { display: none !important; }', 'code[data-unified] :is([data-line-type="change-addition"], [data-line-type="change-addition"] + [data-no-newline]) { display: none !important; }');
    }
    else if (diffSide === "right") {
        rules.push('[data-diff-type="split"] { grid-template-columns: 1fr !important; }', 'code[data-deletions] { display: none !important; }', 'code[data-additions] :is([data-content-buffer], [data-gutter-buffer="buffer"]) { display: none !important; }', 'code[data-unified] :is([data-line-type="change-deletion"], [data-line-type="change-deletion"] + [data-no-newline]) { display: none !important; }');
    }
    return rules.join("\n");
}
...
function createDiffChangeRail(target) {
    ...
    function updateLayout() {
        ...
        rail.style.top = diffView.scrollTop + top + "px";
        rail.style.left = diffView.scrollLeft + Math.max(0, railLeft) + "px";
        ...
        thumb.style.setProperty("--thumb-top", Math.max(0, Math.min(1, scrollTop / scrollHeight)) * 100 + "%");
        thumb.style.setProperty("--thumb-height", Math.max(0.06, Math.min(1, clientHeight / scrollHeight)) * 100 + "%");
    }
...
async function navigateDiff(direction, options = {}) {
...
function updateDiffModeButtons() {
    ...
    diffScrollLockButton.querySelector(".lock-shackle").setAttribute("d", diffScrollLocked ? "M5.5 7V5.25a2.5 2.5 0 0 1 5 0V7" : "M5.5 7V5.25a2.5 2.5 0 0 1 4.75-1.09");
...
function clearReviewedState(options = {}) {
    reviewCommitSha = "";
    reviewKeysByFile = {};
...
function updateChecklistSelectionFromRangeText() {
    ...
    for (const row of checklist.querySelectorAll(".picker-row")) {
...
function applyRangeInputs(options = {}) {
    markInputsValid();
...
function deferRangeInputsUntilAfterSelectionPaint(options = {}) {
...
function scheduleRangeInputs(options = {}) {
...
async function requestOverview(options = {}) {
...
function updateReviewedStateInTree(changedFiles = null) {
    ...
    reviewedTreeUpdateHandle = requestAnimationFrame(() => {
        ...
        for (const button of fileTree.querySelectorAll(".file-row")) {
            button.classList.toggle("reviewed", reviewedPaths.has(button.dataset.path));
        }
        for (const summary of fileTree.querySelectorAll(".tree-folder > summary")) {
...
function renderTreeNode(node) {
    ...
    const files = node.files
        ...
        const fileStatus = button.querySelector(".file-status");
        const filePath = button.querySelector(".file-path");
...
function updateSelectedFileInTree(button = null) {
    ...
    const nextButton = button || Array.from(fileTree.querySelectorAll(".file-row"))
...
function clearStaleFileTree(message = "Loading files...", options = {}) {
    currentFiles = [];
    selectedFileButton = null;
    selectedTreeElement = null;
...
function clearStaleDiff(message = "Loading diff...", options = {}) {
    currentDiff = "";
...
async function loadSelectedFileDiff(options = {}) {
    cancelPendingDiffWork();
...
async function loadDiffData(options = {}) {
    deferredRangeApplyVersion++;
    cancelPendingDiffWork();
    cancelPendingReviewedStateWork();
